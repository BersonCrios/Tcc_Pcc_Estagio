...
import br.com.berson.locationar.utils.KalmanLatLong;
...
public class DeviceLocation implements LocationListener {
    ...
    public void onLocationChanged(final Location newLocation) {
        Log.d(TAG, "MUDOUUUUU (" + newLocation.getLatitude() + "," + newLocation.getLongitude() + ")");
...
//                locationManager.addGpsStatusListener(this);
    ...
    private boolean filterAndAddLocation(Location location) {
        ...
        if (age > 5 * 1000) {
            Log.d(TAG, "A localização é antiga");
            oldLocationList.add(location);

            if (locationScene.isDebugEnabled())
                Toast.makeText(locationScene.mContext, "Rejeitado: antigo", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (location.getAccuracy() <= 0) {
            Log.d(TAG, "Os valores de latitude e longitude são inválidos");
            if (locationScene.isDebugEnabled())
                Toast.makeText(locationScene.mContext, "Rejeitado: Lat ou LOng INválidos", Toast.LENGTH_SHORT).show();
        ...
        if (horizontalAccuracy > getMinimumAccuracy()) { //10meter filter
            Log.d(TAG, "A precisão é muito baixa.");
        ...
        Log.d(TAG, "A qualidade da localização é boa o suficiente.");
